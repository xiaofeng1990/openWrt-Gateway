...
int ipc_init(struct tGatewayInfo *gatewayInfo)
{
	...
	struct mq_attr attr;		/* 消息队列的属性结构体，如果为NULL，就使用默认属性 */
	attr.mq_maxmsg = 10;		/* 定义消息队列的最大消息数量 */
	attr.mq_msgsize = 512;		/* 定义消息队列的单个消息大小 */
	...
	gatewayInfo->mqueue.mqid_report = mq_open(MQ_REPORT, O_CREAT | O_RDWR, 0666, &attr);
	if (gatewayInfo->mqueue.mqid_report == (mqd_t)-1)
	{
		goto err1;
	}

	gatewayInfo->mqueue.mqid_device = mq_open(MQ_DEVICE, O_CREAT | O_RDWR, 0666, &attr);
	if (gatewayInfo->mqueue.mqid_device == (mqd_t)-1)
	{
		goto err1;
	}
	
	gatewayInfo->mqueue.mqid_scene = mq_open(MQ_SCENE, O_CREAT | O_RDWR, 0666, &attr);
	if (gatewayInfo->mqueue.mqid_scene == (mqd_t)-1)
	{
		goto err1;
	}

	gatewayInfo->mqueue.mqid_timer = mq_open(MQ_TIMER, O_CREAT | O_RDWR, 0666, &attr);
	if (gatewayInfo->mqueue.mqid_timer == (mqd_t)-1)
	{
		goto err1;
	}

	gatewayInfo->mqueue.mqid_execute = mq_open(MQ_EXECUTE, O_CREAT | O_RDWR, 0666, &attr);
	if (gatewayInfo->mqueue.mqid_timer == (mqd_t)-1)
	{
		goto err1;
	}
