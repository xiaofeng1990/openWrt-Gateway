...
int mqtt_connect(struct tGatewayInfo *gatewayInfo)
{
	...
	if(mqtt_sendPacketBuffer(buf, len) >0)
	{
		...
		if (MQTTPacket_read(buf, MQTT_PACKET_SIZE_MAX, &mqtt_getdata) == CONNACK)
		{
			...
			{
				DEBUG_LOG("Unable to connect, return code %d\n", connack_rc);
				return -1;
			}
		}
	}
...
int mqtt_publish(char *pTopic, char *pMessage)
{
	...
	{
		DEBUG_LOG("publish topic %s success\n", pTopic);
		return 0;
	}
	else
	{
		DEBUG_LOG("publish topic error\n");	
		return -1;
	}	
}
 ...
 * @brief  	解析mqtt消息中的api
 *
 * @param   input：srting json字符串		output：api
 *
 * @return  0：成功
 *****************************************************************************/
...
int mqtt_get_api(char* json_data, int *api)
{
	...
	item = cJSON_GetObjectItem(root, "api");
	if (item == NULL)
	{
		cJSON_Delete(root);
		return 3;
