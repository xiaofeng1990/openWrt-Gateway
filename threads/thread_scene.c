...
void *scene_thread_f(void *parameter)
{
	...
	mq_getattr(gatewayInfo->mqueue.mqid_scene, &attr); 		/* 获得消息队列属性 */
	size = attr.mq_msgsize; 								/* 获得消息队列属性中的单个消息大小属性 */

	DEBUG_LOG("*****************scene message queues*********************\n");
	DEBUG_LOG("flag=%ld max #msg=%ld max #bytes/msg=%ld #currently on queue=%ld\n", attr.mq_flags ,attr.mq_maxmsg, attr.mq_msgsize, attr.mq_curmsgs);

	while(1)
	{
		memset(&mq_data, 0, sizeof(struct tMsgData));
		DEBUG_LOG("scene thread wait message queue!!!!!\n");
		ret = mq_receive(gatewayInfo->mqueue.mqid_scene, (char*)&mq_data, size, &prio);
		if(ret > 0)
		{
			DEBUG_LOG("scene thread wait data ok mq receice data len = %d\n", ret);
		}
	}
